...
#define UE_KINDA_SMALL_NUMBER	(1.e-4f)
	...
	static inline double InvSqrt(double F) { return 1.0 / sqrt(F); }
	...
	struct alignas(32) VectorRegister4Double {
		...
		inline VectorRegister2Double GetXY() const { return _mm256_extractf128_pd(XYZW, 0); }
		inline VectorRegister2Double GetZW() const { return _mm256_extractf128_pd(XYZW, 1); }

		inline VectorRegister4Double() = default;

		inline VectorRegister4Double(const VectorRegister2Double& InXY, const VectorRegister2Double& InZW) {
			XYZW = _mm256_setr_m128d(InXY, InZW);
		}

		inline constexpr VectorRegister4Double(VectorRegister2Double InXY, VectorRegister2Double InZW, VectorRegisterConstInit)
			: XY(InXY)
			, ZW(InZW) {
		}

		inline VectorRegister4Double(const __m256d& Register) {
			XYZW = Register;
		}

		inline VectorRegister4Double& operator=(const __m256d& Register) {
			XYZW = Register;
			return *this;
		}
	...
	inline VectorRegister2Double MakeVectorRegister2DoubleMask(uint64 X, uint64 Y) {
		union { VectorRegister2Double Vd; __m128i Vi; } Result;
		Result.Vi = _mm_set_epi64x(Y, X);
		return Result.Vd;
	}

	inline VectorRegister4Double MakeVectorRegisterDouble(uint64 X, uint64 Y, uint64 Z, uint64 W) {
		return VectorRegister4Double(MakeVectorRegister2DoubleMask(X, Y), MakeVectorRegister2DoubleMask(Z, W));
	}

	inline VectorRegister4Double VectorTruncate(const VectorRegister4Double& V) {
		return _mm256_round_pd(V, _MM_FROUND_TO_ZERO | _MM_FROUND_NO_EXC);
	}

	inline VectorRegister4Double VectorDivide(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_div_pd(Vec1, Vec2);
	}

	inline VectorRegister4Double VectorNegateMultiplyAdd(const VectorRegister4Double& A, const VectorRegister4Double& B, const VectorRegister4Double& C) {
		return _mm256_fnmadd_pd(A, B, C);
	}

#define DOUBLE_PI					(3.141592653589793238462643383279502884197169399)

	static const struct VectorRegister4Double Double360 = { {360.0, 360.0, 360.0, 360.0} };
	static const struct VectorRegister4Double DOUBLE_DEG_TO_RAD_HALF = { {(DOUBLE_PI / 180.0) * 0.5, (DOUBLE_PI / 180.0) * 0.5, (DOUBLE_PI / 180.0) * 0.5, (DOUBLE_PI / 180.0) * 0.5} };

	inline VectorRegister4Double VectorMod360(VectorRegister4Double X) {
		VectorRegister4Double Temp = VectorTruncate(VectorDivide(X, Double360));
		VectorRegister4Double DoubleResult = VectorNegateMultiplyAdd(Double360, Temp, X);
		return DoubleResult;
	}
	...
	inline void VectorStoreFloat3(const VectorRegister4Double& Vec, double* Dst) {
		_mm_storeu_pd(Dst, Vec.XY);
		_mm_store_sd(Dst + 2, Vec.ZW);
	}

	inline VectorRegister4Double VectorLoad(const double* Ptr) {
		return _mm256_loadu_pd((double*)Ptr);
	}

	inline void VectorStore(const VectorRegister4Double& Vec, double* Dst) {
		_mm256_storeu_pd(Dst, Vec);
	}

	inline VectorRegister4Double VectorLoadAligned(const double* Ptr) {
	...
	inline VectorRegister4Double VectorMultiply(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
	...
	inline void VectorSinCos(VectorRegister4Double* VSinAngles, VectorRegister4Double* VCosAngles, const VectorRegister4Double* VAngles) {
	...
	inline int VectorMaskBits(const VectorRegister4Double& VecMask) {
		return _mm256_movemask_pd(VecMask);
	}

	inline VectorRegister4Double VectorCompareGT(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_cmp_pd(Vec1, Vec2, _CMP_GT_OQ);
	}

	inline int VectorAnyGreaterThan(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return VectorMaskBits(VectorCompareGT(Vec1, Vec2));
	}

	inline VectorRegister4Double VectorSubtract(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
...
#define OutOfLane(Index0, Index1)	(!InSameLane(Index0, Index1))
	...
	inline VectorRegister4Double PermuteLanes(const VectorRegister4Double& Vec) {
...
#define SHUFFLEMASK2(A0,A1) ((A0) | ((A1)<<1))
	...
	inline VectorRegister4Double SelectVectorSwizzle(const VectorRegister4Double& Vec) {
	...
	template<int Index0, int Index1, int Index2, int Index3>
	inline VectorRegister4Double VectorSwizzleTemplate(const VectorRegister4Double& Vec) {
...
#define VectorSwizzle( Vec, X, Y, Z, W )		VectorSwizzleTemplate<X,Y,Z,W>( Vec )
	...
	inline VectorRegister2Double SelectVectorSwizzle2(const VectorRegister4Double& Vec) {
	...
	template<int Index>
	inline VectorRegister4Double VectorReplicateTemplate(const VectorRegister4Double& Vec) {
...
#define VectorReplicate( Vec, ElementIndex )	VectorReplicateTemplate<ElementIndex>(Vec)
	...
	template<int Lane0, int Lane1>
	inline VectorRegister4Double ShuffleLanes(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		static_assert(Lane0 >= 0 && Lane0 <= 1 && Lane1 >= 0 && Lane1 <= 1, "Invalid Index");
		return _mm256_permute2f128_pd(Vec1, Vec2, PERMUTE_LANE_MASK(Lane0, Lane1));
	}

	template <int Index0, int Index1, int Index2, int Index3>
	inline VectorRegister4Double SelectVectorShuffle(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		if constexpr (InLane0(Index0, Index1) && InLane1(Index2, Index3)) {
	...
	template<int Index0, int Index1, int Index2, int Index3>
	inline VectorRegister4Double VectorShuffleTemplate(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		static_assert(Index0 >= 0 && Index0 <= 3 && Index1 >= 0 && Index1 <= 3 && Index2 >= 0 && Index2 <= 3 && Index3 >= 0 && Index3 <= 3, "Invalid Index");
...
#define VectorShuffle( Vec1, Vec2, X, Y, Z, W )		VectorShuffleTemplate<X,Y,Z,W>( Vec1, Vec2 )
	...
	inline VectorRegister4Double MakeVectorRegisterDoubleMask(uint64 X, uint64 Y, uint64 Z, uint64 W) {
	...
	inline VectorRegister4Double VectorBitwiseXor(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_xor_pd(Vec1, Vec2);
	}

	inline VectorRegister4Double VectorAdd(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_add_pd(Vec1, Vec2);
	}

	inline VectorRegister4Double VectorMax(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_max_pd(Vec1, Vec2);
	}

	template <uint32 ComponentIndex>
	inline double VectorGetComponentImpl(const VectorRegister4Double& Vec) {
		return (((double*)&(Vec.XYZW))[ComponentIndex]);
	}

	template <> inline double VectorGetComponentImpl<0>(const VectorRegister4Double& Vec) { return _mm256_cvtsd_f64(Vec); }
	template <> inline double VectorGetComponentImpl<1>(const VectorRegister4Double& Vec) { return _mm256_cvtsd_f64(_mm256_permute_pd(Vec, 1)); }
	template <> inline double VectorGetComponentImpl<2>(const VectorRegister4Double& Vec) { return _mm_cvtsd_f64(_mm256_extractf128_pd(Vec, 1)); }
	template <> inline double VectorGetComponentImpl<3>(const VectorRegister4Double& Vec) { return _mm_cvtsd_f64(_mm_permute_pd(_mm256_extractf128_pd(Vec, 1), 1)); }

#define VectorGetComponent(Vec, ComponentIndex) VectorGetComponentImpl<ComponentIndex>(Vec)

	inline VectorRegister4Double InternalVectorDot3X_Full(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		VectorRegister4Double Prod = VectorMultiply(Vec1, Vec2);
		VectorRegister4Double Shuf = VectorSwizzle(Prod, 1, 1, 3, 3);
		VectorRegister4Double Sum = VectorAdd(Prod, Shuf);
		Shuf = VectorSwizzle(Prod, 2, 3, 2, 3);
		Sum = VectorAdd(Sum, Shuf);

		return Sum;
	}

	inline double VectorDot3Scalar(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return VectorGetComponent(InternalVectorDot3X_Full(Vec1, Vec2), 0);
	}

	inline constexpr VectorRegister2Double MakeVectorRegister2DoubleConstant(double X, double Y) {
		return VectorRegister2Double{ X, Y };
	}

	inline constexpr VectorRegister4Double MakeVectorRegisterDoubleConstant(VectorRegister2Double XY, VectorRegister2Double ZW) {
		return VectorRegister4Double(XY, ZW, VectorRegisterConstInit{});
	}

	inline constexpr VectorRegister4Double MakeVectorRegisterDoubleConstant(double X, double Y, double Z, double W) {
...
#define UE_DOUBLE_THRESH_QUAT_NORMALIZED		(0.01)
	...
	inline VectorRegister4Double VectorDot4(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		VectorRegister4Double R, T;
		R = VectorMultiply(Vec1, Vec2);
		T = VectorSwizzle(R, 1, 0, 3, 2);
		R = VectorAdd(R, T);
		T = VectorSwizzle(R, 2, 3, 0, 1);
		return VectorAdd(R, T);
	}

	inline VectorRegister4Double VectorAbs(const VectorRegister4Double& Vec) {
		return _mm256_and_pd(Vec, DoubleSignMask());
	}

	inline VectorRegister4Double VectorCompareGE(const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_cmp_pd(Vec1, Vec2, _CMP_GE_OQ);
	}

	inline VectorRegister4Double VectorLoadFloat3(const double* Ptr) {
		return _mm256_maskload_pd(Ptr, _mm256_castpd_si256(DoubleXYZMask()));
	}

	inline VectorRegister4Double VectorLoadDouble1(const double* Ptr) {
		return _mm256_broadcast_sd(Ptr);
	}

	inline VectorRegister4Double VectorReciprocalSqrt(const VectorRegister4Double& Vec) {
		return VectorDivide(DoubleOne, _mm256_sqrt_pd(Vec));
	}

	inline VectorRegister4Double VectorReciprocalSqrtAccurate(VectorRegister4Double Vec) {
	...
	inline VectorRegister2Double VectorSelect(const VectorRegister2Double& Mask, const VectorRegister2Double& Vec1, const VectorRegister2Double& Vec2) {
		return _mm_xor_pd(Vec2, _mm_and_pd(Mask, _mm_xor_pd(Vec1, Vec2)));
	}

	inline VectorRegister4Double VectorSelect(const VectorRegister4Double& Mask, const VectorRegister4Double& Vec1, const VectorRegister4Double& Vec2) {
		return _mm256_xor_pd(Vec2, _mm256_and_pd(Mask, _mm256_xor_pd(Vec1, Vec2)));
	}


	inline VectorRegister4Double VectorQuaternionMultiply2(const VectorRegister4Double& Quat1, const VectorRegister4Double& Quat2) {
		VectorRegister4Double Result = VectorMultiply(VectorReplicate(Quat1, 3), Quat2);
		Result = VectorMultiplyAdd(VectorMultiply(VectorReplicate(Quat1, 0), VectorSwizzle(Quat2, 3, 2, 1, 0)), DOUBLE_QMULTI_SIGN_MASK0, Result);
		Result = VectorMultiplyAdd(VectorMultiply(VectorReplicate(Quat1, 1), VectorSwizzle(Quat2, 2, 3, 0, 1)), DOUBLE_QMULTI_SIGN_MASK1, Result);
		Result = VectorMultiplyAdd(VectorMultiply(VectorReplicate(Quat1, 2), VectorSwizzle(Quat2, 1, 0, 3, 2)), DOUBLE_QMULTI_SIGN_MASK2, Result);

		return Result;
	}

	inline void VectorQuaternionMultiply(VectorRegister4Double* Result, const VectorRegister4Double* Quat1, const VectorRegister4Double* Quat2) {
	...
	inline VectorRegister4Double VectorQuaternionInverse(VectorRegister4Double NormalizedQuat) {
		return VectorMultiply(DOUBLE_QINV_SIGN_MASK, NormalizedQuat);
	}
...
public:
	...
	double DistSquared(const FVector& Other) const {
		const double DeltaX = this->X - Other.X;
		const double DeltaY = this->Y - Other.Y;
		const double DeltaZ = this->Z - Other.Z;
		return (DeltaX * DeltaX) + (DeltaY * DeltaY) + (DeltaZ * DeltaZ);
	}
	FVector GetSafeNormal(double Tolerance = 1.e-8) const {
		const double SquareSum = this->SizeSquared();

		// Check if the vector is long enough to be normalized.
		if (SquareSum > Tolerance) {
			// Calculate the inverse of the length.
			const double Scale = 1.0 / std::sqrt(SquareSum);

			// Return the normalized vector.
			return FVector(this->X * Scale, this->Y * Scale, this->Z * Scale);
		}

		// The vector is too small, return a zero vector.
	...
	inline bool IsNearlyZero(float Value, float ErrorTolerance = UE_SMALL_NUMBER) {
		return std::abs (Value) <= ErrorTolerance;
	}

	inline bool IsNearlyZero(double Tolerance = UE_KINDA_SMALL_NUMBER) const {
		return
			std::abs(X) <= Tolerance
			&& std::abs(Y) <= Tolerance
			&& std::abs(Z) <= Tolerance;
	...
	FVector GetSignVector() const {
		return FVector
			(
				FloatSelect(X, 1.0, -1.0),		// LWC_TODO: Templatize FMath functionality
				FloatSelect(Y, 1.0, -1.0),
				FloatSelect(Z, 1.0, -1.0)
			);
	}
};
...
public:
	...
	FQuat() = default;
	FQuat(double a, double b, double c, double d) : X(a), Y(b), Z(c), W(d) {}
	...
	FQuat(VectorRegister4Double V) {
		VectorStoreAligned(V, reinterpret_cast<double*>(this));
	}

	inline static FQuat MakeFromVectorRegister(VectorRegister4Double V) {
		return FQuat(V);
	}

	FQuat operator*(const FQuat& Q) const {
		FQuat Result;
		VectorQuaternionMultiply(reinterpret_cast<VectorRegister4Double*>(&Result), reinterpret_cast<const VectorRegister4Double*>(this), reinterpret_cast<const VectorRegister4Double*>(&Q));
		return Result;
	}

	FQuat operator*(double Scale) const {
		VectorRegister4Double A = VectorLoadAligned(reinterpret_cast<const double*>(this));
		VectorRegister4Double B = VectorSetFloat1(Scale);
	...
	bool IsNormalized() const {
		VectorRegister4Double A = VectorLoadAligned(reinterpret_cast<const double*>(this));
		VectorRegister4Double TestValue = VectorAbs(VectorSubtract(DoubleOne, VectorDot4(A, A)));
		return !VectorAnyGreaterThan(TestValue, DoubleThreshQuatNormalized);
	}

	//static inline const FQuat Identity = { 0.0, 0.0, 0.0, 1.0 };

	FQuat(const FMatrix& M);

	void Normalize(double Tolerance = UE_SMALL_NUMBER) {
		const VectorRegister4Double Vector = VectorLoadAligned(reinterpret_cast<const double*>(this));

		const VectorRegister4Double SquareSum = VectorDot4(Vector, Vector);
		const VectorRegister4Double NonZeroMask = VectorCompareGE(SquareSum, /*VectorLoadFloat1*/VectorLoadDouble1(&Tolerance));
		...
		VectorRegister4Double Result = VectorSelect(NonZeroMask, NormalizedVector, Double0001);

		VectorStoreAligned(Result, reinterpret_cast<double*>(this));
	}

	inline FQuat Inverse() const {
		//checkSlow(IsNormalized());
		return FQuat(VectorQuaternionInverse(VectorLoadAligned(reinterpret_cast<const double*>(this))));
	}
...
struct alignas(16) FMatrix {
	...
	FMatrix(const FPlane& Row0, const FPlane& Row1, const FPlane& Row2, const FPlane& Row3) {
	...
	inline FVector GetScaledAxis(EAxis InAxis) const {
		switch (InAxis) {
			case EAxis::X:
	...
	inline void RemoveScaling(double Tolerance = UE_SMALL_NUMBER) {
		...
		const double Scale0 = FloatSelect(SquareSum0 - Tolerance, InvSqrt(SquareSum0), 1.0);
		const double Scale1 = FloatSelect(SquareSum1 - Tolerance, InvSqrt(SquareSum1), 1.0);
		const double Scale2 = FloatSelect(SquareSum2 - Tolerance, InvSqrt(SquareSum2), 1.0);
		M[0][0] *= Scale0;
		M[0][1] *= Scale0;
		M[0][2] *= Scale0;
		M[1][0] *= Scale1;
...
static void VectorMatrixMultiply(FMatrix* Result, const FMatrix* Matrix1, const FMatrix* Matrix2) {
...
inline VectorRegister4Double VectorTransformVector(const VectorRegister4Double& VecP, const FMatrix* MatrixM) {
	typedef double Double4x4[4][4];
...
// TODO: What
static bool MatrixInverse(FMatrix* DstMatrix, const FMatrix* SrcMatrix) {
	typedef double Double4x4[4][4];
...
inline bool VectorMatrixInverse(FMatrix* DstMatrix, const FMatrix* SrcMatrix) {
	return MatrixInverse(DstMatrix, SrcMatrix);
}
...
public:
	...
	FQuat Quaternion() const {
		...
		const VectorRegister4Double HalfAngles = VectorMultiply(AnglesNoWinding, DOUBLE_DEG_TO_RAD_HALF);
...
public:
	...
	FVector TransformPosition(SDK::FVector&& V) const {
		return Rotation.RotateVector(Scale3D * V) + Translation;
	}
	FVector TransformPosition(SDK::FVector& V) const {
		return Rotation.RotateVector(Scale3D * V) + Translation;
	}
	FVector TransformPosition(const SDK::FVector& V) const {
		return Rotation.RotateVector(Scale3D * V) + Translation;
	}
	FTransform() = default;

	FTransform(FQuat r, FVector t, FVector s) : Rotation(r), Translation(t), Scale3D(s) {}
	...
	inline bool AnyHasNegativeScale(const FVector& InScale3D, const  FVector& InOtherScale3D) const {
		return  (InScale3D.X < 0.f || InScale3D.Y < 0.f || InScale3D.Z < 0.f
			|| InOtherScale3D.X < 0.f || InOtherScale3D.Y < 0.f || InOtherScale3D.Z < 0.f);
	}

	void ConstructTransformFromMatrixWithDesiredScale(const FMatrix& AMatrix, const FMatrix& BMatrix, const FVector& DesiredScale, FTransform& OutTransform) const;
	...
	void Multiply(FTransform* OutTransform, const FTransform* A, const FTransform* B) const {
		A->IsRotationNormalized();
		B->IsRotationNormalized();
	...
	FVector GetSafeScaleReciprocal(const FVector& InScale, double Tolerance = UE_SMALL_NUMBER) const {
	...
	inline FTransform Inverse() const {
		...
		FVector negTrans = { -Translation.X, -Translation.Y, -Translation.X };
		FVector InvTranslation = InvRotation * (InvScale3D * negTrans);
